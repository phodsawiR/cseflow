...
def _write_status(current: int, total: int, topic: str, topic_type: str,
                  done: list, errors: list, finished: bool = False):
    try:
        with open(_STATUS_FILE, "w", encoding="utf-8") as f:
            json.dump({
                ...
                "finished": finished,
                "current": topic,
                "current_type": topic_type,
                "current_num": current,
                "total": total,
                "done": done[-20:],   # เก็บแค่ 20 ล่าสุด
                "errors": errors,
                "ts": time.time(),
...
VAULT_TOPICS = {
    ...
    "procedures": [
        "Arterial Blood Gas", "Lumbar Puncture",
        "NG Tube Insertion", "Paracentesis", "Thoracentesis",
    ],
    "examinations": [
        "general_appearance_examination",
        "vital_signs_examination",
        "heent_examination",
        ...
        "skin_and_appendages_examination",
        "lung_examination",
    ],
}
...
# ─── Style guide (injected into every analyzer prompt) ───────────────────────

_STYLE_GUIDE = """
=== Style Guide — อ่านง่าย เข้าใจง่าย ===
...
def _generate_procedure_note(topic: str, template: str, kg_context: str) -> str:
    ...
    """
    print(f"  [1/2] knowledge — NotebookLM → researcher fallback...")
    kb, research = _query_knowledge(
        topic,
        "indications, contraindications, equipment, step-by-step procedure, complications, post-procedure care"
    )
    knowledge = _best_knowledge(kb, research)

    print(f"  [2/2] analyzer — synthesize + format procedure note...")
    return call_agent(
        "analyzer",
        system=(
            "คุณคืออาจารย์อายุรศาสตร์ผู้เชี่ยวชาญด้านหัตถการ "
            "สร้างคู่มือการทำ procedure ที่ละเอียด ถูกต้อง และเน้นความปลอดภัย"
        ),
        prompt=f"""สร้าง Obsidian procedure note สำหรับ {topic}
...
=== คำสั่ง ===
- Indications: ระบุ clinical scenarios ที่ต้องทำหัตถการนี้
- Contraindications: แยก Absolute และ Relative ให้ชัดเจน
- Equipment: List อุปกรณ์ที่ต้องเตรียม (รวมถึง local anesthesia ถ้าจำเป็น)
- Procedure Steps: เขียนเป็นขั้นตอน 1, 2, 3... ที่นิสิตแพทย์อ่านแล้วทำตามได้ (เน้น aseptic technique)
- Complications: ระบุภาวะแทรกซ้อนที่พบบ่อยและที่รุนแรง พร้อมวิธีป้องกัน/แก้ไข
- Post-procedure care: สิ่งที่ต้องสั่ง/monitor หลังทำเสร็จ
- Attending Questions: 3 ข้อที่ probe จุดสำคัญของ {topic} — เช่น anatomy ที่ต้องระวัง / landmark / clinical pearl / วิธีแก้ปัญหาเฉพาะหน้า / contraindication ที่มักลืม ห้ามถาม indication ตรงๆ พร้อมคำตอบที่ลึก
- ใส่ [[wikilinks]] ทุกครั้งที่กล่าวถึงโรค หรือ procedure อื่น
- Output เป็น Obsidian Markdown ที่พร้อมใช้ทันที ตาม template
""",
    )
...
_GENERATORS = {
    ...
    "procedures": _generate_procedure_note,
}
...
def build_vault(output_dir: str, topics: dict = None, kg: "ClinicalKG" = None,
    ...
    for topic_type, topic_list in topics.items():
        ...
        for topic in topic_list:
            ...
            _write_status(current, total, topic, topic_type, done_topics, errors)
    ...
    _write_status(total, total, "", "", done_topics, errors, finished=True)
    _clear_status()
